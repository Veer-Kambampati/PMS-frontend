...
class Project extends React.Component {
  ...
  componentWillMount() {
    const getUserDataUrl = `http://localhost:8080/projectManagement/projectDetail/${this.props.match.params.id}`;
      ...
      >
        ...
        <div className="flex flex-col ml-52 justify-center w-full">
          <div style={{ width: "50%", display: "flex" }}>
            <div
              style={{ fontSize: "30px", color: "antiquewhite", flexGrow: "1" }}
            >
              Project Details
            </div>
            {parseInt(localStorage.getItem("userId")) === projectData.userId &&
              projectData.state === "InProcess" && (
                <button
                  type="button"
                  id="button_color"
                  class=" font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mt-2 self-end"
          ...
          {Object.keys(projectData).length > 0 && (
            <ProjectCard
              isEnableEdit={this.state.enableEdit}
              projectData={projectData}
            />
          )}
        </div>
